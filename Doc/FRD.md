Okay, hold up. Let’s get real for a second before we cook this frontend doc.

When you say "enterprise theme," I am seriously hoping you mean *functionally* enterprise—as in high data density, bulletproof reliability, and extreme efficiency. I refuse to let us build a miserable, clunky, 2005-era Oracle or SAP dashboard. We are vibecoding with Next.js; we don't do ugly.

Let's aim for a modern, utilitarian, almost industrial aesthetic. Think Vercel or Linear: clean lines, stark contrasts, and built entirely for speed. It needs to look like a serious tool for serious work, without draining the soul of the warehouse worker using it.

Here is your Enterprise Frontend Requirements Document. Drop this right next to the PRD.

---

# 🎨 Frontend Requirements Document: Enterprise IMS

## 1. UI/UX Core Philosophy (The "Anti-Clunky" Manifesto)

* **High Data Density:** Warehouse managers need to see massive amounts of information at a glance. We use compact table rows, minimal padding, and clear typography.
* **Keyboard-First Navigation:** A true enterprise app allows power users to fly through operations without touching a mouse. We need robust command palettes (Cmd+K) and keyboard shortcuts for saving forms and switching tabs.
* **Zero Layout Shift:** Instantaneous feedback is non-negotiable. When a worker validates a receipt, the UI must update optimistically before the server even responds.
* **Industrial Aesthetic:** A monochromatic base (slate/zinc) with highly intentional accent colors. Red strictly for destructive actions or out-of-stock alerts, green for successful validations, and amber for pending statuses.

## 2. Design System & Stack

* **Framework:** Next.js (App Router) for heavily optimized, server-rendered layouts.
* **Component Library:** Tailwind CSS combined with `shadcn/ui`. It gives us absolute control over the styling while providing accessible, enterprise-grade components (data tables, complex dropdowns, slide-out drawers).
* **State Management:** Since we are likely hooking into a real-time backend, we skip Redux entirely. We rely on the native real-time hooks provided by the database client to keep the UI perfectly synced with the ledger.

## 3. Global Layout Structure

* **Persistent Left Sidebar:** The master navigation hub. It must contain routing to Products, Receipts, Delivery Orders, Inventory Adjustments, Move History, Dashboard, Settings, and the user's Profile .


* **Top App Bar:** Reserved strictly for global context: global search (SKU/Product), notifications, and the current active Warehouse selector.
* **Main Content Area:** A fluid, full-height container that dynamically injects the active route.
...
* 
**KPI Metric Cards:** Top-row widgets displaying critical health metrics: Total Products, Low/Out of Stock warnings, Pending Receipts, and Pending Deliveries .


* 
**Dynamic Filter Bar:** A sticky toolbar allowing users to instantly slice the dashboard by document type, status (Draft, Waiting, Ready, Done), warehouse, or product category .


* **Recent Activity Feed:** A streaming list of the latest ledger entries.

### Data Tables (The Workhorse)

* **Architecture:** Every major list (Products, Move History) uses a highly interactive data table.
* **Features:** Must support server-side pagination, multi-column sorting, and column visibility toggling.
* **Row Actions:** Right-aligned action menus (the three dots) on every row for quick edits or initiating an internal transfer directly from the product list.

### Operation Forms (Slide-Overs & Modals)

* **The UX Rule:** Never navigate the user away from their context just to fill out a form.
* **Implementation:** When creating a new Receipt or Delivery Order, a wide slide-over drawer enters from the right side of the screen.
* 
**Form Mechanics:** Real-time validation, dynamic line-item addition (adding multiple products to a single receipt), and a sticky "Validate" button at the bottom .
