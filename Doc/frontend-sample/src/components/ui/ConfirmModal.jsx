...
export default function ConfirmModal({ isOpen, onClose, onConfirm, title, message, confirmLabel, variant = 'success' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-start pt-[20vh] pointer-events-none">
      <div ref={backdropRef} className="fixed inset-0 bg-black/70 backdrop-blur-md pointer-events-auto" onClick={handleClose} />
      
      <div ref={modalRef} className="relative bg-[#0f1426] border border-border/70 rounded-2xl shadow-2xl max-w-md w-full p-6 pointer-events-auto">
        <div className={`w-12 h-12 rounded-2xl mx-auto flex items-center justify-center ${iconBg}`}>
        ...
        <div className="flex gap-3 justify-center mt-6">
          <button 
            onClick={handleClose}
            className="px-4 py-2 border border-border/70 bg-white/5 hover:bg-white/10 text-slate-300 rounded-full text-sm font-medium transition-colors"
          >
            Cancel
          </button>
          <button 
            onClick={() => { onConfirm(); handleClose(); }}
            className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${btnColor}`}
          >
