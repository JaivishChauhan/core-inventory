...
export default function Toast({ id, type, title, message, onRemove }) {
  ...
  return (
    <div
      ref={toastRef}
      className={`relative bg-[#0f1426] border-l-4 ${borderColor} border border-border/70 shadow-2xl rounded-2xl overflow-hidden min-w-[300px] max-w-[380px] pointer-events-auto`}
    >
      ...
      <div 
        ref={progressRef}
        className={`absolute bottom-0 left-0 h-[3px] w-full bg-current ${iconColor} opacity-60`}
      />
    </div>
  )
}
