...
export default function OtpPage() {
  ...
  return (
    <div className="flex min-h-screen w-full bg-background-dark items-start justify-center pt-[10vh] px-6">
      <div
        ref={containerRef}
        className="panel panel-glow p-8 max-w-[460px] w-full mx-auto relative overflow-hidden"
      >
        <div className="w-16 h-16 rounded-2xl bg-primary/10 border-2 border-primary/30 flex items-center justify-center mx-auto shadow-[0_0_0_10px_rgba(76,201,240,0.08)]">
          {isResetMode ? <Lock className="text-primary" size={28} /> : <Mail className="text-primary" size={28} />}
        </div>

        <h2 className="text-xl font-semibold text-slate-100 text-center mt-5">
          ...
          <>
            <div className="mt-5">
              ...
              <input
                ...
                className="mt-1.5 w-full py-2.5 px-4 text-sm rounded-xl bg-white/5 border border-white/10 focus:border-primary/60 focus:ring-2 focus:ring-primary/30"
            ...
            <div className="flex gap-2.5 justify-center mt-7">
              {code.map((digit, index) => (
                <input
                  ...
                  className="w-12 h-14 text-center text-xl font-bold font-mono bg-white/5 border border-white/10 rounded-xl text-slate-100 focus:outline-none focus:border-primary/60 focus:ring-2 focus:ring-primary/30 transition-colors"
                />
              ))}
            </div>
          </>
        ...
        <button
          ...
          disabled={loading}
          className="w-full h-11 bg-primary hover:bg-primary/90 disabled:opacity-70 disabled:cursor-not-allowed text-[#071018] rounded-full font-semibold text-sm mt-8 transition-all flex items-center justify-center gap-2 shadow-[0_12px_30px_-18px_rgba(76,201,240,0.8)]"
        >
          {loading ? (
            <>
