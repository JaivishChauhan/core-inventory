...
export default function DeliveryPage() {
  ...
  const tbodyRef = useTableScrollReveal([deliveries, loading, viewMode])
  ...
  return (
    <AppShell breadcrumbs={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Operations', href: '#' }, { label: 'Deliveries', href: '/operations/delivery' }]}>
      <div className="flex justify-between items-center mb-2">
        <h1 className="text-2xl font-semibold text-slate-100">Delivery Orders</h1>
      ...
      <div className="panel px-5 py-3 mb-4 flex items-center gap-6 overflow-x-auto">
        ...
        <div className="text-xs font-semibold text-rose-300 bg-rose-500/10 px-2.5 py-1.5 rounded-full border border-rose-500/20">
          {stats.late} Late
        </div>
      </div>
      ...
      <div className="panel p-4 flex flex-wrap items-center gap-3 mb-4 justify-between">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="bg-white/5 border border-white/10 rounded-xl py-1.5 px-3 text-sm focus:outline-none focus:border-primary/60 text-slate-300"
          >
      ...
      {loading ? (
        ...
        <KanbanBoard
          items={deliveries}
          columns={['draft', 'waiting', 'ready', 'done']}
          onCardClick={(item) => navigate(`/operations/delivery/${item.id}`)}
          onStatusChange={async (id, newStatus) => {
            if (newStatus === 'ready') await api.post(`/deliveries/${id}/mark-ready`)
            if (newStatus === 'done') await api.post(`/deliveries/${id}/validate`)
            await loadDeliveries()
          }}
        />
      )}
