...
export default function ProductFormPage() {
  ...
  return (
    <AppShell breadcrumbs={breadcrumbs}>
      <div className="sticky top-0 z-10 bg-[#0f1723]/90 backdrop-blur-sm border-b border-slate-800 pb-4 mb-6 pt-2">
        <div className="flex items-center justify-between max-w-[860px] mx-auto">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/products')} className="w-10 h-10 flex items-center justify-center bg-slate-800 rounded-lg hover:bg-slate-700 text-slate-300 transition-colors">
              <ArrowLeft size={20} />
            </button>
            <h1 className="text-2xl font-bold text-slate-100">{isEdit ? 'Edit Product' : 'New Product'}</h1>
          </div>

          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/products')} className="h-10 px-4 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-sm text-slate-300 transition-colors">
              Cancel
            </button>
            <button
              onClick={handleSubmit}
              disabled={saving || loading}
              className="h-10 px-5 bg-primary hover:bg-primary/90 disabled:opacity-70 text-white rounded-lg text-sm font-semibold transition-colors inline-flex items-center gap-2"
            >
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="max-w-[860px] mx-auto pb-20">
        ...
        {loading ? (
          <div className="rounded-xl border border-slate-800 bg-[#161B22] p-6 text-sm text-slate-400">Loading product form...</div>
        ) : (
          <>
            <section>
              ...
              <div className="grid grid-cols-2 gap-5">
                <div className="col-span-2 flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-lg px-4 py-3 text-base text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-primary"
                    required
                  />
                </div>
                ...
                <div className="col-span-1 flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    className="w-full font-mono bg-slate-900 border border-slate-800 rounded-lg px-4 py-2.5 text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-primary"
                  />
                ...
                <div className="col-span-1 flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={handleChange}
                      className="w-full bg-slate-900 border border-slate-800 rounded-lg px-4 py-2.5 pr-10 text-sm text-slate-100 focus:outline-none focus:border-primary appearance-none cursor-pointer"
                    >
                ...
                <div className="col-span-1 flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder="0.00"
                    className="w-full text-right bg-slate-900 border border-slate-800 rounded-lg px-4 py-2.5 text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-primary"
                  />
                </div>
              </div>
            </section>
            ...
            <section className="mt-8">
              ...
              <div className="grid grid-cols-3 gap-5">
                  ...
                  <>
                    <div className="col-span-1 flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        placeholder="0"
                        className="w-full bg-slate-900 border border-slate-800 rounded-lg px-4 py-2.5 text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-primary"
                      />
                    </div>
                ...
                <div className="col-span-3 bg-slate-900 border border-slate-800 rounded-lg p-4 mt-2">
              ...
              <div className="grid grid-cols-1 gap-5">
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-lg px-4 py-3 text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-primary resize-y"
                  />
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium text-slate-400">Tags</label>
                  <div className="w-full bg-slate-900 border border-slate-800 rounded-lg px-2 py-2 min-h-[46px] flex flex-wrap items-center gap-2">
                    {form.tags.map((tag) => (
